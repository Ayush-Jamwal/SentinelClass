...
db_path = "known_faces"    
student_trackers = {}  
STRIKE_COOLDOWN, PROBATION_TIME = 10, 300 

# THE COMPLETE 6-MODE SUITE
SYSTEM_MODES = {
...
while True:
    ...
    # --- 2.1 YOLO DETECTION & COLOR BOXES ---
    ...
    try:
        ...
        for face_data in results:
            if not face_data.empty:
                ...
                x, y, w, h = int(face_data['source_x'].iloc[0]), int(face_data['source_y'].iloc[0]), int(face_data['source_w'].iloc[0]), int(face_data['source_h'].iloc[0])
                active_faces[name] = (x, y, x+w, y+h)
                
                if name not in student_trackers:
                    student_trackers[name] = {"strikes": 0, "last_seen": now, "history": [], "last_strike": 0, "drink_grace": 0, "phone_start": 0, "probation_end": 0}
    ...
    for name, data in student_trackers.items():
        ...
        # --- 3.1 LOCKDOWN SECURITY (ANTI-VANISH) ---
        if data["probation_end"] > now:
            if name not in active_faces:
                data["probation_end"] += 0.1 
                cv2.putText(frame, f"LOCKDOWN PAUSED: {name} MISSING", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
            else:
                fx1, fy1, fx2, fy2 = active_faces[name]
                cv2.putText(frame, f"LOCKDOWN: {int(data['probation_end']-now)}s", (fx1, fy1-35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
            continue 
        elif data["probation_end"] != 0:
            data["strikes"], data["probation_end"] = 1, 0

        # --- 3.2 SMART WATER BREAK (PAUSES ABSENCE) ---
        ...
        if now < data["drink_grace"]:
            ...
            if name in active_faces:
                fx1, fy1, fx2, fy2 = active_faces[name]
                cv2.rectangle(frame, (fx1, fy1), (fx2, fy2), (0, 165, 255), 3) 
                cv2.putText(frame, f"DRINK: {int(data['drink_grace']-now)}s", (fx1, fy1-35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
            continue

        # --- 3.3 VIOLATION SCANNING ---
        if name in active_faces:
            fx1, fy1, fx2, fy2 = active_faces[name]
            mcx, mcy = (fx1+fx2)/2, (fy1+fy2)/2
            
            # Phone 5s Grace Logic
            phone_near = any(fx1-150 < (px1+px2)/2 < fx2+150 for px1,py1,px2,py2 in active_phones)
            if not active_mode["phone"] and phone_near:
            ...
            # Laptop check
            if not active_mode["laptop"] and not violation and any(fx1-150 < (lx1+lx2)/2 < fx2+150 for lx1,ly1,lx2,ly2 in active_laptops):
                violation = "Unauthorized Computer"
                
            # Whisper/Proximity check
            if not active_mode["whisper"] and not violation:
                for oname, (ox1, oy1, ox2, oy2) in active_faces.items():
                    if oname != name:
                        dist = math.sqrt((mcx - (ox1+ox2)/2)**2 + (mcy - (oy1+oy2)/2)**2)
                        if dist < 150:
        ...
        if violation and (now - data["last_strike"] > STRIKE_COOLDOWN):
            if violation == "Unauthorized Exit":
                data["strikes"] = 3
                data["probation_end"] = now + PROBATION_TIME
            else:
                data["strikes"] += 1
    ...
    if key in [ord('q'), ord('Q')]: break
        ...
        engine.say(f"System switched to {SYSTEM_MODES[key]['name']}"); engine.runAndWait()
